/**
 * JATS 1.2 (NLM Journal Publishing) XML generator for Crossref / PMC deposits.
 * Accepts a structured article object; never invents ISSN/DOI — omit empty fields.
 */

export type JatsAffiliation = {
  id: string | number;
  name: string;
  rorId?: string | null;
  country?: string | null;
...
export type JatsAuthor = {
  ...
  aff?: Array<string | number>;
};

export type JatsFunding = {
  funderName: string;
  awardId?: string | null;
  funderId?: string | null;
};

export type JatsSection = {
  id?: string;
  title: string;
  paragraphs: string[];
};

export type JatsSupplementary = {
  id?: string;
  label?: string;
  href: string;
  title?: string;
...
export type JatsArticleInput = {
  ...
  supplementary?: JatsSupplementary[];
  journal?: {
    title?: string;
    issn?: string | null;
    eissn?: string | null;
...
function splitName(author: JatsAuthor): { surname: string; given: string } {
...
function parseIsoDate(iso?: string | null): { year: string; month: string; day: string } | null {
  if (!iso) return null;
...
function historyDate(dateType: string, iso?: string | null): string {
...
function pubDate(iso?: string | null, pubType = "epub"): string {
  const d = parseIsoDate(iso);
  if (!d) return "";
  return `      <pub-date pub-type="${pubType}">
        <day>${d.day}</day>
        <month>${d.month}</month>
        <year>${d.year}</year>
      </pub-date>`;
...
/**
 * Build a JATS 1.2 XML string from article metadata + body sections.
 */
export function generateJATS(article: JatsArticleInput, journalFallback?: JatsArticleInput["journal"]): string {
  if (!article?.title?.trim()) {
    throw new Error("generateJATS: article.title is required");
  }
  ...
  const affById = new Map(affiliations.map((a) => [String(a.id), a]));
...
${article.supplementary
  ...
  const vol = article.volume != null && article.volume !== ""
    ? `      <volume>${xmlEscape(article.volume)}</volume>`
    : "";
  const iss = article.issue != null && article.issue !== ""
  ...
  // silence unused (aff lookup validates structure for future enrichers)
  void affById;
...
/** Fixture for local verification (not published on the live articles list). */
export const DUMMY_JATS_ARTICLE: JatsArticleInput = {
  id: "MS-DUMMY-0001",
  articleNumber: "e0001",
  doi: "",
  title:
  ...
  sections: [
    ...
    },
    {
      ...
      title: "Methods",
      paragraphs: [
        "No experimental procedures were performed. Structured metadata only.",
      ],
    },
    {
      id: "results",
  ...
  dataAvailability: "Dummy datasets are not deposited; replace with FAIR repository links on publish.",
  dataIdentifiers: [
